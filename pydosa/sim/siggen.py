...
class SigGen:
    ...
    def set(self, nsamples: int, srate: float):
    ...
    def generate_sine(self, freq: float, amplitude: float, units='Vpk'):
        """Generate a sine wave."""
        t = self.times()
        volts_pk = self.scale_sine(amplitude, units)
        return np.sin(2 * math.pi * freq * t) * volts_pk

    def amplitude_modulate(self, wave, mod_freq: float, mod_depth: float):
    ...
    def generate_square(self, freq: float):  # XXX needs checking
    ...
    def generate_dc(self, dc: float):
        """Generate a DC value."""
        # It is better to just add a constant to wave
        n = self.nsamples
        return np.full(n, float(dc))

    def generate_noise(self, noise: float, units: str):
...
def times(nsamples: int, srate: float):
...
def quantize(wave: float, dv: float) -> float:
    """Quantize the values."""
